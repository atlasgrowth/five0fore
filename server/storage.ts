...
export class DatabaseStorage implements IStorage {
  ...
  /**
   * Update the estimated completion time for an order
   * 
   * @param id Order ID
   * @param estimatedCompletionTime New ETA
   * @returns Updated order
   */
  async updateOrderETA(id: string, estimatedCompletionTime: Date): Promise<Order | undefined> {
    console.log(`Updating ETA for order ${id} to ${estimatedCompletionTime.toISOString()}`);
    ...
    try {
      ...
      console.error(`Error updating ETA for order ${id}:`, error);
      return undefined;
    }
  }
  
  /**
   * Count active orders (orders that are not CLOSED or SERVED)
   * 
  ...
  /**
   * Count orders completed since a specific date
   * 
   * @param since Date to count from
   * @returns Number of completed orders
   */
  async countCompletedOrdersSince(since: Date): Promise<number> {
    const result = await db
      .select({ count: sql`count(*)` })
      .from(orders)
      .where(and(
  ...
  /**
   * Count active items by station
   * 
   * @returns Map of station to count of active items
   */
  async countActiveItemsByStation(): Promise<{[station: string]: number}> {
    const result = await db
      .select({
      ...
      .from(orderItems)
      .where(and(
        isNotNull(orderItems.station),
        eq(orderItems.completed, false),
        or(
          isNull(orderItems.status),
          ne(orderItems.status, OrderItemStatus.DELIVERED)
        )
      ))
      .groupBy(orderItems.station);
      
    const stationCounts: {[station: string]: number} = {};
    
    for (const row of result) {
      if (row.station) {
        stationCounts[row.station] = Number(row.count);
      }
    }
  ...
  /**
   * Get average cook times for completed items since a specific date
   * 
   * @param since Date to average from
   * @returns Average cook times in seconds
   */
  async getAverageCookTimeForCompletedItemsSince(since: Date): Promise<{
    avgCookSeconds: number;
    avgTotalSeconds: number;
  } | undefined> {
    try {
      // Average cook time (from fired to ready)
      const cookTimeResult = await db
        .select({
          avgCookSeconds: sql`avg(
            extract(epoch from ${orderItems.readyAt}) - 
            extract(epoch from ${orderItems.firedAt})
          )`
        })
        .from(orderItems)
        .where(and(
          isNotNull(orderItems.firedAt),
          isNotNull(orderItems.readyAt),
          sql`${orderItems.firedAt} >= ${since}`
        ));
      
      // Average total time (from order creation to delivery)
      // Use a simpler join approach instead of alias
      const totalTimeResult = await db
        .select({
          avgTotalSeconds: sql`avg(
            extract(epoch from ${orderItems.deliveredAt}) - 
            extract(epoch from ${orders.createdAt})
          )`
        })
        .from(orderItems)
        .innerJoin(orders, eq(orderItems.orderId, orders.id))
        .where(and(
          isNotNull(orderItems.deliveredAt),
          isNotNull(orders.createdAt),
          sql`${orders.createdAt} >= ${since}`
        ));
        
      const avgCookSeconds = Number(cookTimeResult[0]?.avgCookSeconds || 0);
      const avgTotalSeconds = Number(totalTimeResult[0]?.avgTotalSeconds || 0);
      
      // Only return if we have valid data
      if (avgCookSeconds > 0 || avgTotalSeconds > 0) {
        return {
          // Default to 300 seconds (5 minutes) if no data
          avgCookSeconds: avgCookSeconds > 0 ? avgCookSeconds : 300,
          // Default to 600 seconds (10 minutes) if no data
          avgTotalSeconds: avgTotalSeconds > 0 ? avgTotalSeconds : 600
        };
      }
      
      return undefined;
    } catch (error) {
      console.error('Error calculating average cook time:', error);
      return undefined;
    }
  }
